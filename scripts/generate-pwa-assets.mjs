...
const inputIcon = resolve(root, "public/icon-t.png");
...
const splashBackground = "#CC0000";
const iconBackground = "#CC0000";
...
const runGenerator = (args, { fatal = true } = {}) => {
  const result = spawnSync(process.execPath, [binPath, ...args], {
    stdio: "inherit",
  });
  if (result.error) {
    if (fatal) { console.error(result.error); process.exit(1); }
    console.warn("Generator error:", result.error.message);
    return false;
  }
  if (result.status !== 0) {
    if (fatal) process.exit(result.status ?? 1);
    console.warn(`Generator exited with status ${result.status} (non-fatal, will use sharp fallback).`);
    return false;
  }
  return true;
};
...
runGenerator(splashArgs, { fatal: false });

// Run sharp-based fallback to fill any sizes Puppeteer missed
const fallbackScript = resolve(root, "scripts/generate-missing-splashes.mjs");
if (existsSync(fallbackScript) && existsSync(htmlOut)) {
  const fb = spawnSync(process.execPath, [fallbackScript], {
    stdio: "inherit",
    cwd: root,
  });
  if (fb.error) console.warn("Sharp fallback error:", fb.error.message);
}
